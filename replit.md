...
- **AI Client Onboarding System**: True privacy-first conversational interface at /client-onboarding where AI acts as a GUIDE ONLY, never seeing sensitive data. AI asks qualifying questions (country, business type, industry) and provides **intelligent validation metadata** to dynamically show appropriate form fields with country-specific rules. ALL sensitive data (names, emails, addresses, AND tax IDs like PAN/GST/EIN/SSN/VAT/UTR) collected via encrypted forms, NEVER sent to AI model. **Dynamic Validation Engine**: AI returns field-specific validation rules including regex patterns, length requirements, placeholders, cross-field dependencies (e.g., GSTIN must contain PAN), and calculated prefixes (e.g., GSTIN state codes for India). Real-time validation with dependency-aware revalidation ensures data integrity. System supports 10+ countries (India, USA, UK, UAE, Australia, Canada, etc.) with AI-driven validation adapting to each geography's unique requirements. Complete audit trail stored in database (client_onboarding_sessions, onboarding_messages tables). Updated October 2025 with AI-driven intelligent validation system.
