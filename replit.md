...
- **PKI Digital Signatures for Document Security** (October 2025): Enterprise-grade tamper-proof document verification using RSA-2048 cryptographic signatures. **Why PKI over Blockchain**: 10-100x faster, no gas fees, GDPR compliant (private data), legally recognized (eIDAS/ESIGN Act), no external dependencies. **Implementation Details**:
  - **Document Signing**: SHA-256 hash generation + RSA-2048 signature on upload, stored in `documents` table (documentHash, digitalSignature, signatureAlgorithm, signedAt, signedBy, verificationStatus)
  - **Key Management**: Per-organization RSA key pairs (2048-bit) persisted encrypted in `organizationKeys` table to survive server restarts, preventing signature invalidation
  - **Verification Endpoint**: `POST /api/documents/:id/verify` performs cryptographic integrity check (hash comparison + signature verification)
  - **Security Architecture**: Keys loaded from database with strict mode during verification (errors if keys missing to prevent key regeneration attack), memory cache for performance, AES-256 encryption at rest (planned)
  - **Legal Compliance**: Meets eIDAS (EU) and ESIGN Act (US) requirements for digital signatures in financial/legal documents
  - **Crypto Utilities** (`server/crypto-utils.ts`): generateDocumentHash(), signDocumentHash(), verifySignature(), loadOrganizationKeyPair() with multi-tenant isolation
- **Secure Document Management**: Encrypted storage with PKI digital signatures, authenticated downloads, organization/client-level access control, and tamper-proof verification.
