...
- **Resource Management Suite**: Comprehensive resource allocation planner with user × project grid showing % time allocation, timezone-safe date handling (canonical YYYY-MM-DD format), real-time conflict detection and utilization validation. Skills management system with taxonomy CRUD, category management, permission-based controls (settings.manage), React Hook Form + Zod validation with transform/pipe pattern, and complete stats fallbacks computing local metrics when API unavailable. Backend services include ResourceAllocationService for CRUD operations and SkillService for skill taxonomy, user skill profiles, and matching engine with ranked scoring (proficiency, experience, certifications, endorsements). User Skill Profile UI with tabbed layout, self-service skill CRUD via `/api/users/me/skills` endpoints (server-side identity resolution to prevent privilege escalation), and skill-based auto-suggest via GET `/api/tasks/:taskId/matches` endpoint returning ranked SkillMatch[] with scores. All 15+ API routes secured with proper RBAC permissions.
