...
- **Automatic Chat Title Generation**: Backend-driven auto-title generation happens immediately after first assistant message is saved (server/websocket.ts). No frontend timeout needed. Titles generated via LLM using first user+assistant messages (3-6 words descriptive). System ensures message persistence before title generation to avoid race conditions.
- **Luca Chat Search & Archive**: Full-text search across chat session titles and message content (GET /api/luca-chat-sessions/search?q=query). Archive functionality hides chats from main list while keeping them searchable (PATCH /api/luca-chat-sessions/:id/archive). Archived sessions excluded from default GET /api/luca-chat-sessions unless includeArchived=true.
...
- **Admin Template Deletion**: Admin and Super Admin users have explicit bypass for ALL template deletion (workflows, email templates, message templates, document templates, forms) regardless of subscription-based permission filtering. Non-admin users require corresponding delete permissions via RBAC. Ownership checks still apply: Super Admins can delete global marketplace templates (organizationId=null), Admins can only delete their organization's templates (organizationId matches).
- **21-Day Onboarding Journey**: Complete gamified onboarding system with 21 days of accounting workflow tasks (Week 1: Client Intake & Organization, Week 2: Bookkeeping & Transaction Management, Week 3: Compliance, Reporting & Advanced Features). Each day includes 2 required tasks (100-300 pts) and 0-1 optional tasks (30-100 pts). System features automatic day advancement when all required tasks complete, point accumulation, streak tracking, feature unlocking, and realistic accounting curriculum. API endpoint GET /api/onboarding/progress returns combined data structure with field mapping (requiredForDay → isRequired) for frontend compatibility. Rate limited to 20 task completions per minute with ownership validation, idempotency protection, and graceful degradation for missing templates. Comprehensive E2E tested via Playwright covering full 21-day progression, required vs optional logic, day advancement, persistence, and error handling.
- **Profile Picture Upload**: Full user avatar upload system with multipart/form-data handling via multer. Backend validates image types only (JPEG, PNG, GIF, WebP), enforces 5MB file size limit, and automatically deletes old avatars on replacement. Static file serving via Express middleware at /uploads. Frontend uses React Query for reactive updates - both profile page and sidebar auto-refresh when avatar changes. No page reload needed. Upload endpoint: POST /api/users/me/avatar. Fully E2E tested covering upload, display in profile/sidebar, persistence across navigation, and avatar replacement.
